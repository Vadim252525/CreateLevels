    ...
    public static void open(Stage stage, boolean newProject)
    {
        ...
        canvas.setOnMouseMoved(e -> //Координаты
        {
            ...
            if(x < Logic.canvasWidth && y < Logic.canvasHeight)
            {
                ...
                int it = (y * Logic.canvasWidth) + x;
                if(isClosed && ((
                        !isDefaultCell && (!Logic.masCanvasId[it].equals(Logic.replacementClick) || !Logic.masCanvasColor[it].equals(Logic.replacementColor)) ||
                        isDefaultCell && (!Logic.masCanvasId[it].equals(Logic.replacement) || !Logic.masCanvasColor[it].equals(Color.WHITE))
                    )))
                {
                    if(!isDefaultCell) gc.setFill(Logic.replacementColor);
                    else gc.setFill(Color.WHITE);
                    ...
                    addHistory(new String[] {
                            String.valueOf(x),
                            String.valueOf(y),
                            Logic.masCanvasId[it],
                            Logic.masCanvasColor[it].toString()
                    });
                    masReplace.add(null);
                    masColorReplace.add(null);
                    masIdReplace.add(null);
                    ...
                    {
                        gc.fillText(Logic.replacementClick, x * sizeCell + 5, y * sizeCell + 15);
                        Logic.masCanvasId[it] = Logic.replacementClick;
                        Logic.masCanvasColor[it] = Logic.replacementColor;
                    }
                    else
                    {
                        gc.fillText(Logic.replacement, x * sizeCell + 5, y * sizeCell + 15);
                        Logic.masCanvasId[it] = Logic.replacement;
                        Logic.masCanvasColor[it] = Color.WHITE;
                    }
                }
            }
        });
        ...
        border.setBackground(new Background(bgSceneFill()));
        ...
        exsportButton.setOnAction(e -> ExportLevelScene.open(stage, canvas));
        increaseButton.setOnAction(e ->
        {
            comboPouringIndex = comboPouring.getValue();
            sizeCellVoid(false, stage, border);
        });
        decreaseButton.setOnAction(e ->
        {
            comboPouringIndex = comboPouring.getValue();
            sizeCellVoid(true, stage, border);
        });
        ...
        scene.setOnKeyPressed(e -> {
            if(e.isShiftDown() && e.getCode() == KeyCode.ESCAPE)
                ...
                ExportLevelScene.open(stage, canvas);
        ...
        if(!isWindowCloseRequestEvent)
        {
            isWindowCloseRequestEvent = true;
